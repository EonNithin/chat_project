...
def process_files(subject, codec="libmp3lame"):
    try:
        subject = subject
        # Get device name
        device_name = get_device_name()

        files = glob.glob(os.path.join(media_folderpath, '*.mp4'))
        ...
        for input_mp4 in files:
            ...
            mp4_filepath = os.path.join(folder_path, custom_foldername + ".mp4")
            mp3_filepath = os.path.join(folder_path, custom_foldername + ".mp3")
            transcription_txt_filepath = os.path.join(folder_path, custom_foldername + "_transcription.txt")
            summary_txt_filepath = os.path.join(folder_path, custom_foldername + "_summary.txt")
            quiz_txt_filepath = os.path.join(folder_path, custom_foldername + "_quiz.txt")

            # Check if all required files already exist
            if (os.path.exists(mp4_filepath) and
                    os.path.exists(mp3_filepath) and
                    os.path.exists(transcription_txt_filepath) and
                    os.path.exists(summary_txt_filepath) and
                    os.path.exists(quiz_txt_filepath)):
                print(f"\nSkipping {filename}, all files already processed.\n")
                continue
            ...
            try:
                # Move the original MP4 file to the destination folder
                if not os.path.exists(mp4_filepath):
                    print(f"\nMoving original MP4 file {input_mp4} to {mp4_filepath}...\n")
                    os.rename(input_mp4, mp4_filepath)

                # Process the MP3 file if MP3 is missing
                if not os.path.exists(mp3_filepath):
                    print(f"\nExtracting audio from {mp4_filepath}...\n")
                    video_clip = VideoFileClip(mp4_filepath)
                    audio_clip = video_clip.audio
                    audio_clip.write_audiofile(mp3_filepath, codec=codec)
                    print(f"\nSuccessfully converted {mp4_filepath} to {mp3_filepath}\n")
        ...
        if processed_files_count == 0:
            print("\nAll files have been processed. No new files found.\n")
        else:
            print(f"\nProcessing complete. {processed_files_count} files processed.\n")
